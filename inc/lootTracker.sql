...
--
-- Host: localhost
-- Generation Time: Jun 13, 2011 at 07:02 AM
...
CREATE ALGORITHM=UNDEFINED DEFINER=`ryan`@`%` SQL SECURITY DEFINER VIEW `payoutView` AS select `t1`.`saleID` AS `saleID`,`t1`.`groupID` AS `groupID`,truncate((sum(`t1`.`payout`) / (select count(`participants`.`charID`) from `participants` where (`participants`.`groupID` = `t1`.`groupID`) group by `participants`.`groupID`)),0) AS `perMember` from `saleView` `t1` group by `t1`.`groupID`;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`ryan`@`%` SQL SECURITY DEFINER VIEW `profit-payout` AS select `memberPayout`.`saleID` AS `saleID`,(`saleTotalProfit`.`profit` - sum(`memberPayout`.`payout`)) AS `difference` from (`memberPayout` join `saleTotalProfit` on((`memberPayout`.`saleID` = `saleTotalProfit`.`saleID`))) group by `memberPayout`.`saleID`;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`ryan`@`%` SQL SECURITY DEFINER VIEW `saleTotalProfit` AS select `saleData`.`saleID` AS `saleID`,sum(`saleData`.`profit`) AS `profit` from `saleData` group by `saleData`.`saleID`;
