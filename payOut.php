...
if (filter_has_var(INPUT_POST, 'submitPayed')){
	...
	$data = $DB->qa("
		SELECT *
		FROM memberPayout
		WHERE saleID = ?", array(filter_input(INPUT_POST, 'payout', FILTER_SANITIZE_NUMBER_INT)));
		
	foreach ($data AS $character) {
		echo "
		<input type='checkbox' /> <img style='vertical-align: middle;' src='http://evefiles.capsuleer.de/icons/16_16/icon09_09.png' />
			<a onclick='CCPEVE.showInfo(1377, ".$character['charID'].")'>".$character['name']."</a>: ".number_format($character['payout'])." (".$character['payout'].")<br />"; }
